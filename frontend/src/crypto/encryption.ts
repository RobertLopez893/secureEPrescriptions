...
export class EncryptionModule {
  ...
  static encrypt(container: RecetaContainer, dek: Uint8Array, AAD:Uint8Array) {
  ...
  static decrypt(ciphertextHex: string, ivHex: string, dek: Uint8Array, AAD:Uint8Array): RecetaContainer {
    if(ivHex.length !== 24) throw new Error('IV debe ser de 12 bytes (24 hex)');
    if(dek.length !== 32) throw new Error('DEK debe ser de 32 bytes');
    const aes = gcm(dek, hexToBytes(ivHex), AAD);
